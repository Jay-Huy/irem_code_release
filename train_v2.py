...
parser.add_argument('--hopfield_v2', action='store_true',
                    help='use HopfieldEnergySolverV2 (energy-consistent + r^d gauge)')
parser.add_argument('--tie_mode', default='hard', type=str, choices=['hard', 'soft'],
                    help="v2: 'hard' drops W_v/W_o; 'soft' keeps them + per-head tie penalty")
parser.add_argument('--soft_init', default='random', type=str, choices=['random', 'tied'],
                    help="v2 soft: random init (default) or init at the tie point (control arm)")
parser.add_argument('--tie_gamma', default=0.1, type=float,
                    help='v2 soft: base weight of the per-head tie penalty')
parser.add_argument('--degree', default=1.0, type=float,
                    help='v2: homogeneity degree d  (+1 addition/lowrank, -1 inverse)')
parser.add_argument('--learn_degree', action='store_true',
                    help='v2: make d a learnable scalar')
...
def gen_answer(inp, FLAGS, model, pred, scratchpad, num_steps, create_graph=True):
    ...
    if FLAGS.decoder:
        ...
        v2 = hasattr(model, 'gauge')          # HopfieldEnergySolverV2
        if v2:
            # r = RMS(x) on the RAW input; the network only ever sees x/r  (§2.5)
            x_tokens, r = model.embed_input(inp)
            rd = r ** model.degree
            # replay/random guess arrives in RAW scale -> bring it to the normalised scale
            z = model.embed_latent(pred / rd)
        else:
        ...
        for i in range(num_steps):
            ...
            pred_step = model.decode(z, r) if v2 else model.decode(z)
            with torch.no_grad():
...
def init_model(FLAGS, device, dataset):
    if FLAGS.decoder:
        ...
        if getattr(FLAGS, 'hopfield_v2', False):
            from hopfield_models_v2 import HopfieldEnergySolverV2
            model = HopfieldEnergySolverV2(
                dataset.inp_dim, dataset.out_dim,
                num_heads=num_heads, step_lr=step_lr, beta=beta,
                tie_mode=getattr(FLAGS, 'tie_mode', 'hard'),
                soft_init=getattr(FLAGS, 'soft_init', 'random'),
                degree=getattr(FLAGS, 'degree', 1.0),
                learn_degree=getattr(FLAGS, 'learn_degree', False),
            )
        else:
        ...
        for inp, im in train_dataloader:
            ...
            if FLAGS.decoder:
                ...
                if getattr(FLAGS, 'deep_sup', False):
                    # preds[:, 0] is the initial guess (no gradient) -> skip it
            ...
            tie_R = None
            if getattr(FLAGS, 'hopfield_v2', False) and getattr(FLAGS, 'tie_mode', 'hard') == 'soft':
                tie_R = model.tie_penalty()
                tie_g = model.tie_gamma_schedule(
                    it, getattr(FLAGS, 'tie_gamma', 0.1), getattr(FLAGS, 'num_iterations', 10000))
                loss = loss + tie_g * tie_R
            ...
            if it % FLAGS.log_interval == 0 and rank_idx == 0:
                ...
                if tie_R is not None:
                    kvs['tie_R'] = tie_R.item()          # ~2.0 at random init -> 0.0 when tied
                if getattr(FLAGS, 'hopfield_v2', False):
                    kvs['degree_d'] = float(model.degree.detach())
